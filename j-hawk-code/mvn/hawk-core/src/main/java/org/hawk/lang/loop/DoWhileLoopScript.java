...
import static org.hawk.lang.constant.HawkLanguageKeyWord.doWhileLoop;
...
public class DoWhileLoopScript extends MultiLineScript{
    ...
    */ 
    ...
    public  int createScript
        ...
        do {
            if (scriptMap.get(j) != null && !scriptMap.get(j).isEmpty()) {
                ...
                if (lineDoWhileWhileMatcherMap == null) {
                    throw new Exception("Could not find matching while statement for do at {" + i + "}");
                } else {
                    break;
                }
            }
        ...
        innerDoWhileLoopScript.parseMultiLines(scriptMap, mlc.getStart() + 1, mlc.getEnd() );
        i = j + 1;
